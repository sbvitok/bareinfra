...
func runRootCommand(ctx context.Context, c Opts) error {
	...
	if c.Provider != "" && c.Provider != "bareinfra" {
		return errdefs.InvalidInputf("provider %q is not supported: only the bareinfra provider is supported", c.Provider)
	}

	p, err := bareinfra.NewProvider(rm, c.NodeName, c.OperatingSystem)
	if err != nil {
		return err
	}
